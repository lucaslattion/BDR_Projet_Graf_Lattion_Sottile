...
public class Utilisateur_cache_alimentController {

    private Connection conn;
    private AuthController authController;
    public Utilisateur_cache_alimentController(Connection connection, AuthController authController) {
        conn = connection;
    ...
    public void getMany(Context ctx) throws SQLException {
        if (authController.validLoggedUser(ctx)) {
            ...
            while (rs.next()) {
                Utilisateur_cache_aliment utilisateur_cache_aliment = new Utilisateur_cache_aliment();
                utilisateur_cache_aliment.email = rs.getString("email");
                utilisateur_cache_aliment.anom = rs.getString("anom");
                utilisateur_cache_alimentList.add(utilisateur_cache_aliment);
            }
    ...
    public void create(Context ctx) throws SQLException {
        if(authController.validLoggedUser(ctx)){
            ...
            try (PreparedStatement insertStmt = conn.prepareStatement(
                ...
                insertStmt.setString(1, newUtilisateur_cache_aliment.email);
                insertStmt.setString(2, newUtilisateur_cache_aliment.anom);

                insertStmt.executeUpdate();
                ctx.status(HttpStatus.CREATED);
                ctx.json(newUtilisateur_cache_aliment);
                return;
    ...
    public void update(Context ctx) throws SQLException {
        if(authController.validLoggedUser(ctx)){
            ...
            Utilisateur_cache_aliment updateUtilisateur_cache_aliment = ctx.bodyValidator(Utilisateur_cache_aliment.class)
                    .check(obj -> obj.email != null, "Missing email")
                    .check(obj -> obj.anom != null, "Missing anom")
                    .get();

            PreparedStatement stmt = conn.prepareStatement(
                    "UPDATE utilisateur_cache_aliment SET email = ?, anom = ? WHERE email = ? AND anom = ?");
            stmt.setString(1, updateUtilisateur_cache_aliment.email);
            stmt.setString(2, updateUtilisateur_cache_aliment.anom);
